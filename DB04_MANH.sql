...
SELECT ORDD.ORDER_ID, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS TOTAL_PRICE
FROM ORDER_DETAIL AS ORDD 
JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
WHERE YEAR(ORDER_DATE) = 2025
GROUP BY ORDD.ORDER_ID
HAVING TOTAL_PRICE =
...
FROM(
    SELECT ORDD.ORDER_ID, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS PRICE
    FROM ORDER_DETAIL AS ORDD 
    JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
    JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
    WHERE YEAR(ORDER_DATE) = 2025
...
SELECT CUS.*, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS TOTAL_PRICE
FROM ORDER_DETAIL AS ORDD 
JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
JOIN CUSTOMER AS CUS ON CUS.CUSTOMER_ID = ORDS.CUSTOMER_ID
GROUP BY CUS.CUSTOMER_ID; 

SELECT CUS.CUSTOMER_ID, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS TOTAL_PRICE
FROM ORDER_DETAIL AS ORDD 
JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
JOIN CUSTOMER AS CUS ON CUS.CUSTOMER_ID = ORDS.CUSTOMER_ID
...
HAVING TOTAL_PRICE = (
    SELECT MAX(SUB.PRICE)
    FROM (
        SELECT CUS.CUSTOMER_ID, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS PRICE
        FROM ORDER_DETAIL AS ORDD 
        JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
        JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
        JOIN CUSTOMER AS CUS ON CUS.CUSTOMER_ID = ORDS.CUSTOMER_ID
...
SELECT CUS.CUSTOMER_ID, CUS.CUSTOMER_NAME, SUM(ORDD.ORDER_QUANTITY) AS TOTAL_QUANTITY
FROM ORDER_DETAIL AS ORDD
JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
JOIN CUSTOMER AS CUS ON CUS.CUSTOMER_ID = ORDS.CUSTOMER_ID
GROUP BY CUS.CUSTOMER_ID;

SELECT CUS.CUSTOMER_ID, CUS.CUSTOMER_NAME, SUM(ORDD.ORDER_QUANTITY * PRO.UNIT_PRICE) AS TOTAL_PRICE
FROM ORDER_DETAIL AS ORDD
JOIN PRODUCT AS PRO ON ORDD.PRODUCT_ID = PRO.PRODUCT_ID
JOIN ORDERS AS ORDS ON ORDS.ORDER_ID = ORDD.ORDER_ID
JOIN CUSTOMER AS CUS ON CUS.CUSTOMER_ID = ORDS.CUSTOMER_ID
